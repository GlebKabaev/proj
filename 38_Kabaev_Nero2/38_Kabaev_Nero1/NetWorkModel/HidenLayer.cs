    ...
    class HidenLayer : Layer
    {
        ...
        public override double[] BackwardPass(double[] gr_sums)
        {
            ...
            { //вычисление локального градиента
                ...
                {
                    sum += Neurons[k].Weights[j] * Neurons[k].Derr * gr_sums[k];
                }
                gr_sum[j] = sum;
            }
            ...
            { //корректирование весов
                for (int n = 0; n < numofprevneurons + 1; n++)
                {
                    ...
                    if (n == 0)
                    {
                        deltaw = momentum * lastdeltaweights[i, 0] + learningrate * Neurons[i].Derr * gr_sums[i];
                    }
                    else
                    {
                        deltaw = momentum * lastdeltaweights[i, n] + learningrate * Neurons[i].Inputs[n - 1] * Neurons[i].Derr * gr_sums[i];
                    }
